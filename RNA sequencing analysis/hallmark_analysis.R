...
load("Hallmark.rda", verbose = TRUE)
...
ggplot(res.all, aes(x = NES, y = -log10(p.adjust))) + 
  ...
  theme_bw()
ggsave("volcano_like_plot_hallmark.png", height = 15, width = 6)

## Custom dotplot
top <- res.all %>% filter(p.adjust < 0.01, ) %>% distinct(ID) %>% pull()
...
ggplot(res.sub, aes(x = analysis, y = ID, fill = NES, size = -log(p.adjust))) +
  ...
  labs(x = NULL, y = NULL, title = "GSEA with Hallmark", size = bquote(~-log[10] ~ italic(FDR))) +
  theme_bw() +
  theme(plot.title = element_text(size = 12, face = "bold.italic", hjust = 0.5),
        axis.text.x = element_text(angle = 30, vjust = 0.5))
ggsave("GSEA_with_Hallmark.png", height = 15, width = 10)
...
for(analysis in names(raw)){
  ...
  out_folder <- file.path("pheatmap_Hallmark", make_clean_names(analysis))

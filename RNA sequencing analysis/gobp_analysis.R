...
res.all <- bind_rows(raw.list, .id = "analysis") %>% 
  ...
  mutate( 
    ...
    mylabel  = ifelse(p.adjust < 0.01, ID, "") )
...
ggplot(res.all, aes(x = NES, y = -log10(p.adjust))) + 
  ...
  theme_bw()
ggsave("volcano_like_plot_gobp.png", height = 15, width = 6)


## Custom dotplot
top <- res.all %>% 
  filter(p.adjust < 0.01) %>% 
  filter(NES > 2.5 | NES < -1.7) %>% 
  distinct(ID) %>% 
  pull()

res.sub <- res.all %>%
  filter(ID %in% top, p.adjust < 0.01)
...
ggplot(res.sub, aes(x = analysis, y = ID, fill = NES, size = -log(p.adjust))) +
  ...
  labs(x = NULL, y = NULL, title = "GSEA with GOBP", size = bquote(~-log[10] ~ italic(FDR))) +
  theme_bw() +
  theme(plot.title = element_text(size = 12, face = "bold.italic", hjust = 0.5),
        axis.text.x = element_text(angle = 30, vjust = 0.5))
ggsave("GSEA_with_GOBP.png", height = 15, width = 10)
...
for(analysis in names(raw)){
  ...
  out_folder <- file.path("pheatmap_GOBP", make_clean_names(analysis))
  
  res <- raw[[analysis]]
  
  sig.pathways <- res@result %>% 
    filter(p.adjust < 0.05) %>% 
    filter(ID != "GOBP_ADRENAL_GLAND_DEVELOPMENT") %>% 
    filter(ID != "GOBP_HYDROGEN_PEROXIDE_BIOSYNTHETIC_PROCESS") %>% 
    filter(ID != "GOBP_RESPONSE_TO_FOOD") %>% 
    filter(ID != "GOBP_GANGLION_DEVELOPMENT") %>% 
    filter(ID != "GOBP_SPLICEOSOMAL_TRI_SNRNP_COMPLEX_ASSEMBLY") %>%
    pull(ID)
